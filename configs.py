...
config["sigma_scale_stochasticity"] = [
    np.ones(1),
]
config["prior"] = "gaussian"  # dirichlet, gaussian
config["encoder"] = ["gauss",] * config[
    "nlatents"
]  # det, gaussian
config["decoder"] = ["det",] + ["gauss",] * (
    config["nlatents"] - 1
)  # det, gaussian
...
lrec /= 100
config["lambda_init"] = [
    lrec**n / config["zdim"][n] for n in range(config["nlatents"])
]
...
config[
    "mlpinit"
] = "glorot_uniform"  # normal, he, glorot, glorot_he, glorot_uniform, ('uniform', range)
...
config_mnist["sigma_scale_resample"] = 16.0 * np.ones(1)
config_mnist["sigma_scale_stochasticity"] = [
    eps**2 * np.ones(1) for eps in [0.01, 0.05, 0.1, 0.5, 1.0, 2.0]
]
...
# NN set up
config_mnist["archi"] = ["mlp",] * config[
    "nlatents"
]  # mlp, dcgan
config_mnist["nlayers"] = [
    2,
] * config["nlatents"]
config_mnist["nfilters"] = [2048, 1024, 512, 256, 128]
config_mnist["filters_size"] = [
    3,
] * config["nlatents"]
config_mnist["nonlinearity"] = "elu"  # soft_plus, relu, leaky_relu, tanh
config_mnist["output_layer"] = ["mlp",] * config[
    "nlatents"
]  # dense, conv, conv1x1
config_mnist["updownsample"] = False
...
# Objective set up
config_svhn["coef_rec"] = 1.0  # coef recon loss
...
config_svhn["pz_scale"] = 1.0
config_svhn["prior"] = "gaussian"  # dirichlet or gaussian

# lambda set up
config_svhn["lambda_scalar"] = 10.0
config_svhn["lambda"] = [
    1 / config_svhn["zdim"][i] for i in range(config_svhn["nlatents"])
]
...
config_celebA["zdim"] = [8, 6, 4, 8, 6, 4, 8, 6, 4, 128]

# Sigma setup
config_celebA["sigma_scale_resample"] = 16.0 * np.ones(1)
config_celebA["sigma_scale_stochasticity"] = [
    eps**2 * np.ones(1) for eps in [0.01, 0.05, 0.1, 0.5, 1.0, 2.0]
]
...
# lambda set up
config_celebA["lambda_scalar"] = 10.0
config_celebA["lambda"] = [
    1 / config_celebA["zdim"][i] for i in range(config_celebA["nlatents"])
]
...
config_celebA["encoder"] = ["gauss"] * config_celebA[
    "nlatents"
]  # deterministic, gaussian
config_celebA["decoder"] = ["det",] + ["gauss",] * (
    config_celebA["nlatents"] - 1
)  # deterministic, gaussian
config_celebA["arch"] = ["dcgan"] * config_celebA["nlatents"]  # mlp, dcgan, ali, began
config_celebA["last_archi"] = [
    "conv1x1",
] * config_celebA["nlatents"]
config_celebA["nlayers"] = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2]
config_celebA["filters_size"] = [7, 5, 3, 3, 3, 3, 3, 3, 3, 3]
config_celebA["nfilters"] = [64, 64, 64, 64, 96, 96, 96, 128, 128, 128]
config_celebA["nonlinearity"] = "leaky_relu"  # soft_plus, relu, leaky_relu, tanh
config_celebA["updownsample"] = [
    True,
    False,
    False,
    True,
