...
def Encoder(
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        if archi == "mlp":
            ...
            outputs, out_shape = dcgan_encoder(
                opts,
                input,
                nlayers,
                nfilters,
            ...
            outputs, out_shape = dcgan_v2_encoder(
                opts,
                input,
                nlayers,
                nfilters,
            ...
            outputs, out_shape = resnet_v2_encoder(
                opts,
                input,
                input_shape,
                nlayers,
    ...
    Sigma = tf.nn.softplus(logSigma)
    return (
        tf.compat.v1.layers.flatten(mean),
        tf.compat.v1.layers.flatten(Sigma),
        out_shape,
    )


def mlp_encoder(
    opts, input, nlayers, nunits, output_dim, reuse=False, is_training=False
):
    layer_x = input
...
def dcgan_v2_encoder(
    ...
    if layer_x.get_shape().as_list()[1:-1] != features_dim[:-1]:
        layer_x = ops.linear.Linear(
            ...
            layer_x,
            np.prod(layer_x.get_shape().as_list()[1:]),
            np.prod(features_dim),
            scope="hid_linear_init",
        )
        layer_x = tf.reshape(
            layer_x,
            [
    ...
    if opts["enorm"] == "batchnorm":
        ...
        layer_x = ops.layernorm.Layernorm(opts, layer_x, "hid%d/bn" % (i + 1), reuse)
    layer_x = ops._ops.non_linear(layer_x, opts["nonlinearity"])
    # Flaten layer
