...
def log_normal(z, mean, sigma, eps=1e-6):
    ...
    return -0.5 * (
        c + tf.compat.v1.log(eps + sigma) + tf.square(z - mean) / (1e-5 + sigma)
    )
...
def KL(mu0, cov0, mu1, cov1, eps=1e-6):
    ...
    kl = (
        (cov0) / (eps + cov1)
        + tf.square(mu1 - mu0) / (eps + cov1)
        + tf.compat.v1.log((eps + cov1) / (eps + cov0))
        - 1
    )
...
def mc_kl_penalty(samples, q_mean, q_Sigma, p_mean, p_Sigma):
    ...
    """
    kl = (
        tf.compat.v1.log(q_Sigma)
        - tf.compat.v1.log(p_Sigma)
...
def sinkhorn_it_v2(opts, C):
    ...
    # Initialization
    u = opts["epsilon"] * (
        tf.compat.v1.log(M) - logsumexp(-C / opts["epsilon"], axis=1, keepdims=True)
    )
    v = opts["epsilon"] * (
        tf.compat.v1.log(M)
        - logsumexp((-C + u) / opts["epsilon"], axis=0, keepdims=True)
    )
    Sinkhorn = []
    ...
    for l in range(opts["L"] - 1):
        u -= opts["epsilon"] * (
            tf.compat.v1.log(M)
            + logsumexp((-C + u + v) / opts["epsilon"], axis=1, keepdims=True)
        )
        v -= opts["epsilon"] * (
            tf.compat.v1.log(M)
            + logsumexp((-C + u + v) / opts["epsilon"], axis=0, keepdims=True)
        )
...
def mmd_penalty(opts, sample_qz, sample_pz):
    ...
    if opts["mmd_kernel"] == "RBF":
        ...
        stat = 0.0
        for scale in [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0]:
            res1 = (1.0 + distances_qz / scale / 2.0) ** (-scale)
            res1 += (1.0 + distances_pz / scale / 2.0) ** (-scale)
            res1 = tf.multiply(res1, 1.0 - tf.eye(n))
            res1 = tf.reduce_sum(res1) / (nf * nf - nf)
            res2 = (1.0 + distances / scale / 2.0) ** (-scale)
            res2 = tf.reduce_sum(res2) * 2.0 / (nf * nf)
            stat += res1 - res2
...
def square_dist_dotprod(opts, sample_x, sample_y):
    ...
    squared_dist = (
        norms_x
        + tf.transpose(norms_y)
        - 2.0 * tf.matmul(sample_x, sample_y, transpose_b=True)
    )
...
def mahalanobis_cost_v2(x1, x2, mu, Sigma):
    ...
    cost = (
        tf.matmul(tf.transpose(xdiff, perm=[0, 1, 3, 2]), cost)
...
def cov(x, mu, axis):
    ...
    if axis == 0:
        ...
        vx = tf.matmul(
            tf.transpose(x, perm=[1, 2, 0]), tf.transpose(x, perm=[1, 0, 2])
        ) / tf.cast(tf.shape(x)[0], tf.float32)
        return vx - mx
    elif axis == 1:
        vx = tf.matmul(tf.transpose(x, perm=[0, 2, 1]), x) / tf.cast(
