...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    if ((axes == [0,2,3]) or (axes == [0,2])) and fused==True:
        ...
        # offset = tf.Variable(name+'.offset', np.zeros(mean.get_shape()[-1], dtype='float32'))
        # scale = tf.Variable(name+'.scale', np.ones(var.get_shape()[-1], dtype='float32'))
        ...
        offset = tf.Variable(name+'.offset', np.zeros(inputs.get_shape()[1], dtype='float32'))
        scale = tf.Variable(name+'.scale', np.ones(inputs.get_shape()[1], dtype='float32'))

        moving_mean = tf.Variable(name+'.moving_mean', np.zeros(inputs.get_shape()[1], dtype='float32'), trainable=False)
        moving_variance = tf.Variable(name+'.moving_variance', np.ones(inputs.get_shape()[1], dtype='float32'), trainable=False)
        ...
        if 0 not in axes:
            ...
            shape[0] = 1
        offset = tf.Variable(name+'.offset', np.zeros(shape, dtype='float32'))
        scale = tf.Variable(name+'.scale', np.ones(shape, dtype='float32'))
