...
def ConvMeanPool(
    opts, input, input_dim, output_dim, filter_size, scope=None, init="he", biases=True
):
    output = Conv2d(
        opts,
        input,
...
def UpsampleConv(
    ...
    output = input
    output = tf.concat(
        [output, output, output, output], axis=-1
...
def ResidualBlock(
    ...
    output = input
    output = Batchnorm_layers(
        opts, output, scope=scope, is_training=is_training, reuse=reuse
    )
    output = ops._ops.non_linear(output, "relu")
    output = conv_1(opts, input=output, scope=scope + "/Conv1", filter_size=filter_size)
    output = Batchnorm_layers(
        opts, output, scope=scope + "/Bnv1", is_training=is_training, reuse=reuse
    )
    output = ops._ops.non_linear(output, "relu")
...
def OptimizedResBlockEnc1(opts, input, output_dim):
    ...
    conv_2 = functools.partial(
        ConvMeanPool, input_dim=output_dim, output_dim=output_dim
    )
