...
# run setup
parser.add_argument(
    "--model", default="stackedwae", help="model to train [vae/wae/lvae/stackedwae]"
)
...
parser.add_argument(
    "--grid", action="store_false", default=True, help="plot pz grid interpolation"
)
parser.add_argument(
    "--stoch",
    action="store_false",
...
def main():
    ...
    if FLAGS.dataset=='mnist':
        ...
        opts["lambda_init"] = [
            lrec * log(n + 1.0001) / opts["zdim"][n] for n in range(0, opts["nlatents"] - 1)
        ] + [
            lmatch / 100,
        ]
        opts["lambda"] = [
        ...
        # pen_sigma_coef = [9.0 / 10.0, 10.0 / 10.0, 11.0 / 10.0]
