...
from loss_functions import Xentropy_penalty, entropy_penalty, kl_penalty, mc_kl_penalty, vae_sigmoid_reconstruction_loss, moments_loss
...
class VAE(object):
    ...
    def __init__(self, opts):
        ...
        for n in range(opts['e_nlatents']):
            ...
            self.enc_mean.append(enc_mean)
            self.enc_Sigma.append(enc_Sigma)
            ...
            if n==0:
                ...
                if n<opts['e_nlatents']-1:
                    self.match_penalty += mc_kl_penalty(self.encoded[-2],
                                                self.enc_mean[-2], self.enc_Sigma[-2],
                                                dec_mean, dec_Sigma)
                else:
    ...
    def train(self, data, WEIGHTS_FILE):
        ...
        for epoch in range(opts['epoch_num']):
            ...
            for it in range(batches_num):
                ...
                [_, loss, loss_rec, match_reg, kl_reg] = self.sess.run([
                                                self.wae_opt,
                                                self.objective,
                ...
                Loss.append(loss)
                Loss_rec.append(loss_rec)
                Loss_match.append(match_reg+kl_reg)
                Match_reg.append(match_reg)
                KL_reg.append(kl_reg)
                ...
                if counter % opts['print_every']==0 or counter==100:
                    ...
                    logging.error(debug_str)
                    debug_str = 'REC=%.2f, REC TEST=%.2f' % (
                                                Loss_rec[-1],
                                                Loss_rec_test[-1])
                    logging.error(debug_str)
                    debug_str = 'MATCH=%10.2e, LATENT REG=%10.2e, KL REG=%10.2e\n ' % (
                                                Loss_match[-1],
                                                Match_reg[-1],
                                                KL_reg[-1])
                    logging.error(debug_str)
