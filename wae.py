...
class WAE(object):
        ...
        for n in range(opts['e_nlatents']):
            ...
            if opts['encoder'][n] == 'det':
                ...
                if opts['pen_enc_sigma']:
                    pen_enc_sigma += opts['lambda_pen_enc_sigma'][n]*tf.reduce_mean(tf.reduce_sum(tf.abs(tf.log(enc_Sigma)),axis=-1))
                # Enc Sigma stats
    ...
    def train(self, data, WEIGHTS_FILE):
        ...
        decay, counter = 1., 0
        decay_steps, decay_rate = int(batches_num * opts['epoch_num'] / 10), 0.98
        ...
        for epoch in range(opts['epoch_num']):
            ...
            for it in range(batches_num):
                ...
                if counter % opts['print_every']==0 or counter==100:
                    ...
                    [reconstructed_test, encoded, samples] = self.sess.run(
                                                ...
                                                feed_dict={self.points:data.test_data[:10*npics],
                                                           ...
                                                           self.dropout_rate: 1.,
                                                           self.is_training:False})
                    reconstructed_test = [data.test_data[:10*npics],] + reconstructed_test
                ...
                if counter >= batches_num * opts['epoch_num'] / 10 and counter % decay_steps == 0:
